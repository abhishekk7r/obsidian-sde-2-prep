...
>-  Second would be Appstore runs on physical devices so it's not like moving a backend stateless service. So,  so we typically push the end points of the new regions where the OTA (Over the Air), in a similar way that you get updates on your phone. We have a config service, which dev
>
