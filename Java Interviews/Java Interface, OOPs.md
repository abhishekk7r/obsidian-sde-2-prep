...
Why: Entry sits in the bucket from INSERT time. Mutating the field changes the hash, so get() now searches a DIFFERENT bucket than where the entry actually lives.
